...
public protocol Reducer<State, Action> {
    ...
    ///
    /// When you create a custom reducer by implementing the ``body``, Swift infers this type
    /// from the value returned.
    ///
    /// If you create a custom reducer by implementing the ``reduce(into:action:)``, Swift
    ...
    ///
    /// > Important: if your reducer implements the ``reduce(into:action:)`` method, it will
    /// > take precedence over this property, and only ``reduce(into:action:)`` will be called
...
extension Reducer where Body: Reducer<State, Action> {
    /// Invokes the ``Body``'s implementation of ``reduce(into:action:)``.
    @discardableResult
    @inlinable
    public func reduce(into state: inout Body.State,
                       action: Body.Action) -> Effect<Body.Action> {
